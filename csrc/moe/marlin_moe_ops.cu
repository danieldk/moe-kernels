...
void marlin_mm_moe(const void* A, const void* B, void* C,
                   ...
                   void* workspace, moe_kernels::ScalarType const& q_type,
...
torch::Tensor marlin_gemm_moe(
    ...
    moe_kernels::ScalarTypeId const b_q_type_id, int64_t size_m, int64_t size_n,
  ...
  moe_kernels::ScalarType const b_q_type = moe_kernels::ScalarType::from_id(b_q_type_id);
  bool has_zp = b_zeros.size(1) != 0;
  if (has_zp) {
    TORCH_CHECK(
        b_q_type == moe_kernels::kU4,
        "b_q_type must be u4 when has_zp = True. Got = ", b_q_type.str());
  } else {
    TORCH_CHECK(
        b_q_type == moe_kernels::kU4B8 || b_q_type == moe_kernels::kU8B128,
